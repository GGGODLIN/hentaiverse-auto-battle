...
function sendNotification(title, body) {
  try {
    ...
    console.error("[RMA] Notification failed", e);
  }
}
...
function parseResponseHeaders(headerStr) {
  ...
  while ((p = headerStr.indexOf("\r\n", (i = p + 2) + 5)) > i)
    (q = headerStr.indexOf(":", i + 3)) > i &&
      q < p &&
      (headers[(k = headerStr.slice(i, q).toLowerCase())] = headerStr.slice(
        q + 2,
        p
      ))[0] === '"' &&
...
GM_registerMenuCommand("查看剩餘次數", async function () {
  var remaining = await GM.getValue("ratelimit_remaining", "尚未查詢過");
  sendNotification("Riddle Master", "今日剩餘次數: " + remaining);
});
...
if (document.getElementById("riddlecounter")) {
  ...
  window.addEventListener("load", function (event) {
    //need to wait for riddlebot to complete download
        ...
        GM.xmlHttpRequest({
          ...
          onload: async function (response) {
            ...
            if (return_dict.return == "error") {
              ...
              GM.setValue("ratelimit_remaining", responseheaders["x-ratelimit-remaining"]);
              sendNotification("Riddle Master", "今日剩餘次數: " + responseheaders["x-ratelimit-remaining"]);
              ...
              if (document.hasFocus()) {
                setTimeout(function () {
                  console.log(
                    "[RMA]INFO: Auto submit with delay of " +
                      extend_submit_interval +
                      " secs"
                  );
              ...
              if (return_dict.expire == true) {
                GM_notification(
                  "WARNING: License is expired/invalid",
                  "Riddle Master Assistant Reborn"
                );
              }
            } else {
              GM_notification(
                "ERROR: Server respond with unknown result",
                "Riddle Master Assistant"
              );
          ...
          onerror: function (response) {
            ...
            if (response.status == 429) {
              GM_notification(
                "ERROR: All solves used up for this IP today",
                "Riddle Master Assistant"
              );
            } else {
              GM_notification(
